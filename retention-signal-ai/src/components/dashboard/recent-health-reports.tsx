...
interface RecentHealthReportsProps {
  rows: HealthScoreRow[];
  loading: boolean;
}
...
export function RecentHealthReports({ rows, loading }: RecentHealthReportsProps) {
  // Already sorted newest first from the hook
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-sm font-semibold">Recent Health Reports</CardTitle>
        </div>
      </CardHeader>
        ...
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Upload a weekly report to generate your first health score.
            </p>
          </div>
        ) : (
          <Table>
            ...
            <TableBody>
              {recent.map((row) => (
                <TableRow key={row.id} className="hover:bg-muted/50">
                  ...
                  <TableCell>
                    <span className={`text-sm font-bold ${getScoreColor(row.overall_health_score)}`}>
                  ...
                  <TableCell className="text-xs text-muted-foreground font-mono">
                    {row.report_id ? row.report_id.slice(0, 12) + "…" : "—"}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
